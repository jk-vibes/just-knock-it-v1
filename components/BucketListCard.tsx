...
export const BucketListCard: React.FC<BucketListCardProps> = ({ 
  ...
  const getStyles = () => {
      switch (theme) {
          ...
          case 'elsa':
              return {
                  card: 'bg-gradient-to-br from-white via-cyan-50 to-sky-100 border-cyan-100 shadow-cyan-100/50',
                  text: 'text-cyan-950',
  ...
  return (
    <div className="relative group overflow-hidden rounded-2xl mb-1.5 active:scale-[0.98] transition-transform duration-200">
      <div 
        className="absolute inset-0 bg-red-600 flex items-center justify-end pr-8"
        onClick={handleDelete}
      >
        <div className="flex flex-col items-center gap-1 text-white animate-in slide-in-from-right-4 duration-300">
      ...
      <div 
        id={`card-${item.id}`}
        className={`${s.card} border rounded-2xl p-3 shadow-sm relative z-10 transition-all duration-300 ${isHighlighted ? 'ring-4 ring-offset-2 ring-red-500 scale-[1.02] z-20' : 'hover:-translate-y-1'}`}
        style={{ 
            transform: `translateX(${offsetX}px)`,
            transition: isSwiping ? 'none' : 'transform 0.4s cubic-bezier(0.175, 0.885, 0.32, 1.275)'
        }}
        onClick={() => offsetX === 0 ? onEdit(item) : setOffsetX(0)}
