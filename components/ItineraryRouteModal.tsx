...
import { ArrowLeft, MapPin, Navigation, Plus, Trash2, Save, Map as MapIcon, List as ListIcon, Sparkles, Loader2, Footprints, MoreVertical, Zap, Flag, GripVertical, Star, Clock, X, Info, Circle, CheckCircle2, AlertTriangle, ChevronUp, ChevronDown, BookOpen, GraduationCap } from 'lucide-react';
    ...
    return (
        <div className="absolute inset-0 z-[20] bg-white dark:bg-zinc-950 flex flex-col">
            <div className="absolute top-4 left-4 z-[1000]">
                <button onClick={onClose} className="bg-white dark:bg-zinc-900 p-3 rounded-full shadow-lg border border-gray-200 dark:border-zinc-800 text-gray-700 dark:text-white">
                    ...
                    <div className="flex overflow-x-auto gap-4 no-scrollbar snap-x snap-mandatory py-4">
                        {stops.map((stop, idx) => (
                            <div 
                                ...
                                className={`snap-center shrink-0 w-[85%] max-w-sm bg-white dark:bg-zinc-800 p-4 rounded-2xl shadow-xl border-2 transition-all cursor-pointer ${activeStopIndex === idx ? `border-[${themeColor}] scale-100` : 'border-transparent scale-95 opacity-80'}`}
...
const StopDetailsModal = ({ stop, onClose, travelMode = 'driving' }: { stop: ItineraryItem, onClose: () => void, travelMode?: TravelMode }) => {
    return (
        <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm animate-in fade-in duration-200" onClick={onClose}>
            <div className="bg-white dark:bg-zinc-900 rounded-2xl w-full max-w-md shadow-2xl overflow-hidden flex flex-col max-h-[85vh]" onClick={e => e.stopPropagation()}>
                ...
                <div className="p-6 overflow-y-auto">
                    ...
                    {stop.bestVisitingTime && (
                        <div className="flex items-center gap-2 mb-4 text-xs font-medium text-orange-600 dark:text-orange-400 bg-orange-50 dark:bg-orange-900/20 px-3 py-2 rounded-lg border border-orange-100 dark:border-zinc-800/50">
                            <Clock className="w-4 h-4" />
                            <span>Target: {stop.bestVisitingTime}</span>
                        </div>
                    )}
                        ...
                        <div className="mb-6">
                            ...
                            <div className="flex flex-wrap gap-2">
                                {stop.interests.map((tag, i) => (
                                    <span key={i} className="px-2 py-1 rounded bg-gray-100 dark:bg-zinc-800 text-gray-600 dark:text-zinc-300 text-[10px] font-bold border border-gray-200 dark:border-zinc-700">
                                        #{tag}
                                    </span>
                                ))}
                            </div>
...
export const TripPlanner: React.FC<TripPlannerProps> = ({ item, onClose, onUpdateItem, onAddSeparateItem, userLocation, theme, travelMode = 'driving' as TravelMode, distanceUnit = 'km' }) => {
  ...
  const labels = useMemo(() => {
    if (item?.type === 'goal') return { header: 'Learning Path', subItem: 'Milestone', placeholder: 'Add a learning milestone...', info: 'Focus on sequential growth steps.' };
    if (item?.type === 'roadtrip') return { header: 'Route Stops', subItem: 'Waypoint', placeholder: 'Add a scenic stop...', info: 'Focus on landmarks along the route.' };
    return { header: 'City Itinerary', subItem: 'Spot', placeholder: 'Add a local spot...', info: 'Focus on gems within the city area.' };
  }, [item?.type]);
  ...
  const s = useMemo(() => {
    const orange = {
        ...
        moon: 'bg-red-600 text-white shadow-lg shadow-red-900/20'
    };

    switch (theme) {
        case 'moon': return { 
            bg: 'bg-zinc-950', surface: 'bg-zinc-900', header: 'bg-zinc-950 border-white/5', text: 'text-white', textDim: 'text-zinc-400', accent: 'text-red-500', border: 'border-white/5', line: 'bg-zinc-800', dot: 'bg-red-600 border-zinc-950', badge: orange.moon, btnPrimary: 'bg-red-600 hover:bg-red-700 text-white shadow-red-900/20', btnGhost: 'bg-white/5 text-zinc-300 hover:text-white hover:bg-white/10', cardHover: 'hover:border-white/10 hover:shadow-black/40',
        };
        case 'marvel': return { 
  ...
  const handleMagicFill = async () => {
      ...
      setErrorMsg(null);
      try {
          const suggestions = await generateItineraryForLocation(item);
          ...
          if (filteredSuggestions.length > 0) {
            ...
            onUpdateItem({ ...item, itinerary: updated });
              ...
              setErrorMsg(`No more new ${labels.subItem.toLowerCase()}s found.`);
          }
      } catch (error) {
  ...
  const handleStopCompletionConfirm = (date: number) => {
      ...
      const newCompletedItem: BucketItem = {
          ...
          description: completingStop.description || `Milestone reached for ${item?.title}`,
          type: item?.type === 'goal' ? 'goal' : 'destination',
          locationName: item?.locationName,
          coordinates: completingStop.coordinates,
          completed: true,
          completedAt: date,
          createdAt: Date.now(),
          category: completingStop.category || item?.category || 'Personal Growth',
  ...
  if (viewMode === 'map' && item.type !== 'goal') {
      return (
          <RouteMap 
            stops={stops} 
  ...
  return (
    <div className={`absolute inset-0 z-0 flex flex-col ${s.bg}`}>
            ...
            <div className="flex justify-between items-start gap-3">
                <div className="flex-1 min-w-0 pt-0.5">
                    ...
                    <div className="flex items-center flex-wrap gap-2">
                         ...
                         <div className={`flex items-center gap-3 text-[10px] font-medium ${s.textDim}`}>
                            {item.locationName && item.type !== 'goal' && <span className="truncate max-w-[80px]">{item.locationName}</span>}
                            {item.type !== 'goal' && (
                            ...
                            <span className="flex items-center gap-1">
                                {item.type === 'goal' ? <BookOpen className="w-3 h-3" /> : <Flag className="w-3 h-3" />}
                                {stops.length} {labels.subItem}{stops.length !== 1 ? 's' : ''}
                            </span>
                        </div>
                    </div>
                </div>
                ...
                <div className="flex gap-1.5 shrink-0">
                     <button 
                        ...
                        title={`Magic Fill ${labels.subItem}s`}
                    >
        ...
        <div className="flex-1 overflow-y-auto p-6 relative no-scrollbar">
            {errorMsg && (
                <div className="mb-4 p-3 rounded-xl bg-orange-50 dark:bg-orange-900/20 border border-orange-100 dark:border-zinc-800 flex items-center gap-2 text-xs font-bold text-orange-600 dark:text-orange-400 animate-in slide-in-from-top duration-300">
            ...
            <div className="flex gap-4 relative mb-2">
                ...
                <div className="pb-8">
                    <h3 className={`text-sm font-bold ${s.text}`}>
                        {item.type === 'goal' ? 'Initialization' : 'Start Point'}
                    </h3>
                    <p className={`text-xs ${s.textDim}`}>
                        {item.type === 'goal' ? 'Path creation started' : (item.locationName || 'Destination Center')}
                    </p>
                </div>
            </div>
            ...
            {stops.map((stop, idx) => (
                <div key={`${stop.name}-${idx}`} className="flex gap-4 relative group animate-in slide-in-from-bottom-2 fade-in">
                    <div className="flex flex-col items-center pt-1.5">
                        <div className={`w-8 h-8 rounded-full border-4 flex items-center justify-center transition-all duration-300 z-10 ${stop.completed ? 'bg-green-500 border-green-100 text-white' : 'bg-white dark:bg-zinc-800 ' + s.border + ' ' + s.text}`}>
                        ...
                        >
                            ...
                            <div className="flex flex-col items-center gap-2 shrink-0">
                                <button 
                                    ...
                                    className="p-1.5 text-zinc-500 hover:text-red-500 hover:bg-red-50 dark:hover:bg-red-900/20 rounded-lg transition-all"
                                >
            ...
            <div className="flex gap-4 relative">
                <div className="flex flex-col items-center">
                    <div className={`w-8 h-8 rounded-full border-4 flex items-center justify-center bg-zinc-100 dark:bg-zinc-800 ${s.border} ${s.textDim}`}>
                         {isAdding ? <Loader2 className="w-4 h-4 animate-spin" /> : <Plus className="w-4 h-4" />}
                    </div>
                </div>
                <div className="flex-1 pb-12">
                    <div className={`flex items-center gap-2 p-1.5 pr-2 rounded-2xl border bg-white dark:bg-zinc-800 ${s.border}`}>
                        <input 
                            type="text" 
                            value={newPlace}
                    ...
                    <p className="text-[10px] font-bold text-gray-400 mt-2 px-1 flex items-center gap-1">
                        <Info className="w-3 h-3" /> {labels.info}
                    </p>
                </div>
            </div>
        </div>
